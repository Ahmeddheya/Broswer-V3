...
interface BrowserState {
  ...
  // Tabs management (unified from tabsStore)
...
export const useBrowserStore = create<BrowserState>((set, get) => ({
  ...
  closeTab: (tabId: string) => {
    ...
    if (tabToClose) {
      ...
      set((state) => ({
        ...
        closedTabs: [...state.closedTabs, closedTab],
  ...
  closeAllActive: () => {
    ...
    set((state) => ({
      activeTabs: [],
      closedTabs: [...state.closedTabs, ...closedTabs],
      currentTabId: undefined,
    }));
  ...
  updateTabUrl: (id: string, url: string) => {
    ...
    set((state) => ({
      activeTabs: state.activeTabs.map(tab =>
        tab.id === id ? { ...tab, url: resolvedUrl, title } : tab
      ),
    }));
