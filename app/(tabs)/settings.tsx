...
export default function SettingsScreen() {
  ...
  const handleSettingChange = async (section: string, key: string, value: unknown) => {
    setAdvancedSettings(prev => ({
      ...prev,
      [section]: {
    ...
    if (section === 'core') {
      await updateSetting(key as keyof AdvancedBrowserSettings, value as AdvancedBrowserSettings[keyof AdvancedBrowserSettings]);
    }
  };

  const handleBasicSettingChange = async (key: string, value: unknown) => {
    setAdvancedSettings(prev => ({
      ...prev,
      [key]: value,
    }));
  ...
  const renderSettingItem = (item: { key: string; title: string; value: unknown; type: string; options?: string[] }, index: number) => {
    return (
      <TouchableOpacity 
        key={index} 
        style={[
